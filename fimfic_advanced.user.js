...
function updatePinned(target, pinClass) {
    ...
    if (!$('.fix_' + pinClass).length) {
        if (min >= $(target).offset().top) {
            $('body').attr('data-' + pinClass + '-pos', $(target).offset().top);
            $('body').addClass('fix_' + pinClass);
        }
    } else {
        if (min < parseInt($('body').attr('data-' + pinClass + '-pos'))) {
            $('.fix_' + pinClass).removeClass('fix_' + pinClass);
        }
    }
}
function updatePinnedWithMax(target, pinClass, bounder) {
    ...
    var min = window.scrollY + ($('.pin_nav_bar').length ? 45 : 0);
    if (!$('.fix_' + pinClass).length && window.scrollY < max) {
        if (min >= $(target).offset().top) {
            $('body').attr('data-' + pinClass + '-pos', $(target).offset().top);
            $('body').addClass('fix_' + pinClass);
        }
    } else {
        if (window.scrollY > max || min < parseInt($('body').attr('data-' + pinClass + '-pos'))) {
            $('body').removeClass('fix_' + pinClass);
        }
    }
}
...
function makeList(element, ordered) {
    ...
    for (var i = 0; i < len; i++) {
        if (ordered) {
            if (selected[i].indexOf('\t[b]·[/b] ') == 0) {
                selected[i] = selected[i].replace('\t[b]·[/b] ', '\t[b]' + (i + 1) + '.[/b] ');
                toggle = false;
            } else if (selected[i].indexOf('\t[b]' + (i + 1) + '.[/b] ') != 0) {
...
function applyFeedFix() {
    $(document).on('click', '.feed_body img.thumbnail_image', function(e) {
        e.preventDefault();
...
function previewSig() {
    return fillBBCode(sign({}, pickNext([
        ...
        'Iets ryk soos kaas.',
        'I am best Pony!!!',
        'Your argument is invalid. <img class="user_image" alt="" src="//fc01.deviantart.net/fs71/f/2014/013/d/b/cslyra_by_comeha-d7220ek.png"></img>',
        'U is bestest pone!',
...
function sign(target, text) {
    if (text == null || text == undefined) text = target.value;
    ...
    for (var i = 0; i < tags.length; i++) formatted = replaceAll(tags[i][0], tags[i][1], formatted);
    if (text == null || text == undefined) text = target.value;
...
function listNames() {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if  (name != null && name != "") {
            ...
            bbCode += "#" + (start + i + 1) + "  [url=//www.fimfiction.net" + link + "]" + name + "[/url]\n";
            name = censorStory(stories[i]) ? '--hidden--' : name;
            sfw += "#" + (start + i + 1) + "  " + name + "\n";
            sfwCode += "#" + (start + i + 1) + "  " + (name == '--hidden--' ? name : "[url=//www.fimfiction.net" + link + "]" + name + "[/url]") + "\n";
        }
    }
    ...
    var toggleSFW = $('<input checked="" type="checkbox" />');
    ...
    function upd() {
        if (toggle[0].checked) {
            area.val(toggleSFW[0].checked ? sfwCode : bbCode);
        } else {
            area.val(toggleSFW[0].checked ? sfw : result);
        }
    }
}
...
function addBannerCredits(items) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        item.append(banner);
        var source = $('<div class="source" style="background-color:' + items[i].colour + '"><input type="radio" value="' + items[i].id + '" name="banners[]" /></div>');
        item.append(source);
        if (items[i]['source']) {
...
@media all and (min-width: 700px) {\
    ...
    .fix_userbar header.header {\
       margin-bottom: 40px;}\
    .pin_nav_bar.fix_userbar .user_toolbar {\
       top: 45px;}}\
\
...
.fix_feed .content > .content_background > .inner {\
    margin-top: 55px;}\
...
.pin_nav_bar.fix_userbar.fix_feed .feed-toolbar {\
    top: 85px;}\
...
function getPinUserbar() {return settingsMan.getB('pin_userbar', false);}
function setPinUserbar(v) {settingsMan.setB('pin_userbar', v);}

function getResizeVideos() {return settingsMan.getB('resize_vids', true);}
function setResizeVideos(v) {settingsMan.setB('resize_vids', v);}

function getAlwaysShowImages() {return settingsMan.get('unspoiler_images', '1') == '1';}
function setAlwaysShowImages(val) {settingsMan.set('unspoiler_images', val ? '1' : '0');}

function getSweetieEnabled() {return settingsMan.getB("sweetie_staff_enabled", false);}
function setSweetieEnabled(val) {settingsMan.setB("sweetie_staff_enabled", val);}

function getSlide() {return settingsMan.int("slideShow", 0);}
function setSlide(v) {settingsMan.set("slideShow", v);}

function getSig() {return settingsMan.get("user_sig", defSig());}
function setSig(v) {settingsMan.set("user_sig", v);}
function defSig() {return "%message%\n\n--[i]%name%[/i]";}
...
function setSaveFocus(v) {settingsMan.setB('ultra_snow_save_focus', v);if (snower) snower.SetSaveFocus(v);}
...
function getFancyBanners() {return settingsMan.getB('fancy_banners', false);}
function setFancyBanners(v) {
    settingsMan.setB('fancy_banners', v);
    if (v) {
        bannerScrollOn();
        updateBannerScroll();
    } else {
...
function setStoryWidth(v) {settingsMan.set("storyWidth", v);}
...
function getCustomBanner() {
    if (settingsMan.has("customBannerUrl") && settingsMan.has("customBannerColor") && settingsMan.has("customBannerPosition")) {
        var url = settingsMan.get("customBannerUrl");
        var color = settingsMan.get("customBannerColor");
        var pos = settingsMan.get("customBannerPosition");
        if (url == '-none-' || color == '-none-' || pos == '-none-') return null;
...
function getBannersEnabled() {return settingsMan.getB("banners", true);}
...
function setBGColor(c) {settingsMan.set("bgColor", c);return c;}
function getBGIndex() {return settingsMan.int("bgImg", -1);}
function setBackgroundImg(v) {settingsMan.set("bgImg", v);}
...
//==API FUNCTION==//
function urlSafe(me) {
    var result = me.toLowerCase().replace(/['",;.?]/gi,'').replace(/[^a-z0-9_-]/gi, '-');
...
function Slider() {
    var slideTimes = [-1, 60000, 180000, 300000, 600000, 1800000, 3600000];
    ...
    var me = {
        ...
        updateSlide: function() {
            ...
            if (slide > 0 && fade) {
                isSliding = true;
                slideshowTimer = setTimeout(transitionStart, getTime(slide));
            }
        },
    ...
    function loadImg() {
        $('.user_toolbar > ul').css("transition-duration", "3s");
        chooseTheme(theme, true);
        setTimeout(transitionEnd, 3000);
        fade.css({"transition": "opacity 3s linear", "opacity": "0"});
        isSliding = false;
        me.updateSlide();
    }
    ...
    function transitionStart() {
        ...
        $('<img>').attr('src', banners[theme].url).load(loadImg);
    }

    function transitionEnd() {$('.user_toolbar > ul').css("transition-duration", "");}
    function getTime(v) {return slideTimes[v < 0 && v >= slideTime.length ? 0 : v];}
...
function setupSweetie() {
    ...
    var baseCursorUpgradeCost = 10;
    var cursorUpgradeCost = 0;
    var baseCursorCost = 4;
    var cursorCost = baseCursorCost;
        ...
        if (event.keyCode == 32) {
            if ($(belle).attr("dragging") == "true") {
                ...
                if (passLevel == 4) {
                    ...
                    no = true;
                    if (grabbedImage == null) {
                        if (Math.floor(Math.random() * 10) % 2 == 0) doImg();
                        ...
                        $(grabbedImage).css('top', $(window).scrollTop() + parseFloat($(grabbedImage).css('top')));
                        $(grabbedImage).css('left', $(window).scrollLeft() + parseFloat($(grabbedImage).css('left')));
...
.wobbly_image {\
    -{0}-animation: shake_shake 0.25s linear infinite;}\
...
@-{0}-keyframes wobble {\
    ...
    to {margin-left: 10px;}}\
.musical {\
    -{0}-animation: wub 0.8s linear 0s infinite alternate;\
@-{0}-keyframes wub {\
    ...
    function doImg() {
        ...
        settingsMan.set('sweetie_img_index', index);
    }
    ...
    function doGrab() {
        ...
        var img = $(document.elementFromPoint(lastClientX , lastClientY));
        belle.style.left = oldX;
        if (img != null && img[0].tagName == "IMG" && !img.hasClass('nopickup')) {
            if (img.attr("dragged") != "true") {
                ...
                img.addClass('grabbedImage');
                img.parent().append(img);
                img.css({
                    'top' : (lastY - img.height()/2) + 'px',
                    'left' : (lastX - img.width()/2) + 'px',
                });
                grabbedImage = img[0];
            }
        }
    ...
    function plusOne(x, y, text) {
        ...
        $('body').append(one);
        one.css('top', (y - one.height()) + 'px');
        one.css('left', (x - one.width()) + 'px');
        setTimeout(function() {
            one.css('opacity', 0);
            setTimeout(function() {
                one.remove();
        ...
        while ($('.floater').length > 30) {
            $('.floater').first().remove();
        }
        return one;
    }
    ...
    function spawnCursor() {
        ...
        $(document).one('mouseup.cursor', function(event) {
            cursor.find('cursor').removeClass('click');
            $(document).off('mousemove.cursor');
            checkClick(cursor.find('.cursor'));
            event.preventDefault();
        });
    }

    function checkClick(cursor) {
        var ref = cursor.offset();
        var cookies = $('.cookie');
        var clicks = 5;
        var distance = 100 + (50 * parseInt(cursor.attr('data-level')));
        for (var c = 0; c < cookies.length && clicks > 0; c++) {
            var i = $(cookies[c]);
            var coo = i.offset();
            
            var cookieX = coo.left + i.width()/2;
            var cookieY = coo.top + i.height()/2;

            var difX = ref.left - cookieX;
            var difY = ref.top - cookieY;
            
            var dist = Math.sqrt(difX*difX + difY*difY);
            if (dist <= distance) {
                i.trigger('mousedown', {pageX: cookieX, pageY: cookieY});
                clicks--;
            }
        }
    ...
    function buildShop(holder) {
        var totalCursors = $('.cursor').length > 0;
        if (bank >= cursorCost) {
            var button = $('<button class="styled_button">Buy 1 Cursor (' + cursorCost + ')</button>');
            ...
            button.on('click', function() {
                if (bank >= cursorCost) {
                    ...
                    spawnCursor();
                    holder.empty();
                    holder.parent().css('opacity', 0);
                }
            });
        }
        if (totalCursors > 0 && cursorUpgradeCost > 0 && bank >= cursorUpgradeCost) {
            button = $('<button class="styled_button">Upgrade Cursors (' + cursorUpgradeCost + ')</button>');
            holder.append(button);
            button.on('click', function() {
                bank-= cursorUpgradeCost * totalCursors;
                $('.cursor').each(function() {
                    var i = $(this);
                    var level = parseInt(i.attr('data-level')) + 1;
                    if (level < 6) {
                        i.attr('data-level', level);
                        i.parent().find('.level').html(level);
                        i.parent().find('.value').html(level * baseCursorCost);
                        if (level == 5) {
                            cursorUpgradeCost -= baseCursorUpgradeCost * (level - 1);
                        } else {
                            cursorUpgradeCost += baseCursorUpgradeCost;
                            i.css('transform', 'scale(' + level + ',' + level + ')');
                        }
                    }
                });
                say(getPointsName() + ': ' + bank + '</br>' + getPointsName() + ' Clicked: ' + score);
...
function onYouTubePlayerReady(playerId) {\
    document.getElementById("cookie_song").addEventListener("onStateChange", "playerEnded");\
    $(document).trigger("playerReady");\
}\
function playerEnded(s) {\
    if (s == 0) $(document).trigger("playerEnded");\
}</script><span></span></div>');
                    }
                    if (!$('#cookie_song').length) {
                        $('#song_frame span').html('<object id="cookie_song" height="0" width="0" type="application/x-shockwave-flash" data="//www.youtube.com/v/eaK5_eJRzmA?autoplay=1&enablejsapi=1&playerapiid=cookie_song&version=3"><param name="allowScriptAccess" value="always" /></object>');
                        $(document).one('playerEnded', function() {
                            $('#song_frame span').html('');
                            $(belle).removeClass('musical');
                        });
                        $(document).one('playerReady', function() {
                            $(belle).addClass('musical');
                        });
                    }
                }
            }
            say('Cookies: ' + bank + '</br>Cookies Clicked: ' + score);
            var max = $(belle).hasClass('musical') ? 3 : 1;
            for (var i = 0; i < max; i++) {
                if (Math.random() * 16 < max) {
    ...
    function help() {
        var pop = makePopup(jule('wSeeit ecSpeet reSrcte snIedx'), 'fa fa-child', false);
        pop.SetWidth(700);
        var content = 'iPkcpuD/or pmiga;ePSCA Eo(ev rmiga)eW-bolb emIgaseS;IHTFS+APECC-ooik elCciek;r+CPSCA-EeHrastH;S+APECS-ahekS;ahekV girouolsy';
    ...
    function updateSelection(index) {
        if ($('#belle_type').length) {
            ...
            me.next().html('<span style="transition:opacity 0.5s linear">' + imgLabels[index] + '</span>');
    ...
    function shake() {
        if ((Math.floor(Math.random()*31)) == 5) {
            ...
            $("body").css('animation', 'shake_shake 0.17s linear ' + count);
            setTimeout(function() {
                $("body").css('animation', '');
            },count * 170);
        }
    ...
    function say(text, duration, fadeOptions) {
        var timeout = $('#belle .speech').attr('data_timeout_function');
        if (timeout != null && timeout != '') {
            clearInterval(parseInt(timeout));
        }
        $('#belle .speech_container').css('opacity', text == '' ? 0 : 1);
        $('#belle .speech').html(text);
        if (duration != null) {
            $('#belle .speech').attr('data_timeout_function', setTimeout(function() {
                $('#belle .speech_container').css('opacity', 0);
                $('#belle .options_container').css('opacity', 0);
                if (fadeOptions) {
                    $('#belle .speech').attr('data_timeout_function', '');
                }
            },duration));
        }
    }
    ...
    function options(factory) {
        var container = $('#belle .options');
        container.html('');
        factory(container);
        $('#belle .options_container').css('opacity', container.children().length ? 1 : 0);
    }
