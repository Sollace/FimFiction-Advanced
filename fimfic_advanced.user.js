...
function catchBanners(id) {
    ...
    var before = banners[id == 0 ? banners.length - 1 : id - 1];
    var after = banners[(id + 1) % banners.length];
...
function getBookmark(num) {return settingsMan.int(num + '_bookmark_position', 0);}
function setBookmark(num) {
    settingsMan.set(num + '_bookmark_position', '0');
    settingsMan.set(num + '_bookmark_d', $('.story-page-header h1.resize_text a').first().text() + '\n' + $('#chapter_title').text());
...
function getFullBookmark(num) {
    ...
    entry.key = num;
    if (num.indexOf(':') != -1) {
        entry.bookmark = ['published'].concat(num.split(':'));
    } else {
        entry.bookmark = ['unpublished'].concat(num.split('_'));
    }
    entry.init = function(data) {
        ...
        if (data != null) {
            ...
            this.url.chapter = '//www.fimfiction.net/' + (this.bookmark[0] == 'published' ? 'story/' + this.bookmark[1] + '/' : 'chapter/') + this.bookmark[2] + (this.bookmark[0] == 'published' ? '/' + this.url.title + '/' + this.url.chapTitle : '');
        } else {
            this.url.title = 'Unknown Story';
            this.url.chapTitle = 'Unknown Chapter';
            this.url.story = this.url.chapter = '';
        }
        return this;
    }
    return entry.init(getBookmarkData(num));
}
function getBookmarkData(name) {
    var result = {};
    if (settingsMan.has(name + '_bookmark_d')) {
