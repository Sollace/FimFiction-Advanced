...
var settingsMan = {
    ...
    getB: function(key, def) {return this.has(key) ? ((key = this.__get(key)) == 'true' || key == '1' || !!key) : def;},
    int: function(key, def) {return this.has(key) ? parseInt(this.__get(key)) : def;},
    float: function(key, def) {return this.has(key) ? parseFloat(this.__get(key)) : def;},
...
var extraBanners = [
    ...
    Ban2("mommy_derp", "//junglepony.deviantart.com/art/Derpy-Mom-326785301", "rgb(189,198,205)"),
...
function patchEvents() {
    function override(obj, member, new_func) {
        if (obj[member].super) obj[member] = obj[member].super;
...
function initGlobals() {
    userToolbar = document.querySelector('.user_toolbar');
}
...
function initFimFictionAdvanced() {
    ...
    applyCodePatches();
    if (document.querySelector('.right-menu-inner, #browse_form')) addStoryList();
...
function pinnerFunc(target, pinClass, bounder) {
    ...
    return function() {
        if (bounder) {
            var max = offset(bounder).top + bounder.offsetHeight - 100;
            if (window.scrollY >= max) {
                document.body.classList.remove('fix_' + pinClass);
                return;
            }
        }
        var min = window.scrollY + (document.body.classList.contains('pin_nav_bar') ? 45 : 0);
...
function applyChapterButtons() {
    jSlim.all('ul.chapters', addChapterButtonsExtras);
    jSlim.on(document, 'a.compact_chapters', 'click', function() {
        this.parentNode.parentNode.classList.remove('all-shown');
    });
    jSlim.on(document, 'a.compact.min', 'click', function() {
        var me = this.parentNode.parentNode;
        me.classList.toggle('chapters_compact');
        this.innerHTML = me.classList.contains('chapters_compact') ? 'Maximize' : 'Minimize';
    });
    jSlim.on(document, 'a.compact.max', 'click', function() {
        var me = jSlim.nearest(this, '.chapters_compact');
        if (me) {
            me.classList.remove('chapters_compact');
            me.querySelector('a.compact.min').innerHTML = 'Minimize';
        }
    });
}

function addChapterButtonsExtras(me) {
    me = $(this);
    var loggedIn = getIsLoggedIn();
    var chapters = me.find('li .chapter-read-icon').length;
    var compact = chapters > 1;
    me.find('.chapter_expander').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        me.parent().toggleClass('all-shown');
    }).parent().addClass('chapter-expander-toggle');
    if (compact || loggedIn) {
        ...
        if (compact) {
            ...
            extra.append('<a class="compact min" style="float:right;" href="javascript:void(0);" >Minimize</a>');
            ...
            extra.after('<div class="all-chapters-hidden">' + chapters + ' chapters hidden. <a class="compact max" style="display:inline;" href="javascript:void(0);" >Show</a></div>');
        }
    }
}
...
function applyFeedFix() {
    jSlim.on(document, '.feed_body img.thumbnail_image', 'click', function(e) {
        e.preventDefault();
        var a = jSlim.nearest(this, 'a');
...
function loopUnspoiler() {
    ...
    if (!getExtraEmotesInit()) {
        jSlim.all('.comment_data a.user_image_link', unspoilerSiblings);
    }
}

function initImageUnspoiler() {
    jSlim.all('.comment_data .user_image_link', function(me) {
        me.parentNode.insertBefore(jSlim.newEl('IMG', { 'class': 'user_image', 'data-lightbox': '', src: me.href }));
        me.parentNode.parentNode.removeChild(me.parentNode);
    });
}

function lightboxblocker() {
    jSlim.on(document.body, '[data-lightbox]', 'mouseover', function(e) {
        if (this.classList.contains('user_image')) {
            this.removeAttribute('data-lightbox');
            this.removeAttribute('title');
            this.dataset.resizeable = false;
            this.classList.remove('user_image');
            this.classList.add('user_image_no_lightbox');
        }
        this.classList.add('no-lightbox');
    });
}

function unspoilerSiblings(me) {
    var url = me.href.replace(/$(https:|http:)/g,'');
    me = me.parentNode;
    if (mustUnspoiler(url)) {
        me.outerHTML += '<img class="user_image" src="' + url + '"></img>';
        me.parentNode.removeChild(me);
        logger.Log("unspoilerSiblings: " + url);
        return;
    }
    if (me.nextSibling && me.nextSibling.tagName != 'BR') {
        me.parentNode.insertBefore(me.nextSibling, document.createElement('BR'));
    }
}
...
function buildAdvancedButton(button, controller) {
    ...
    function handleIndent(e, start, end) {
        ...
        var g = !1;
        "" == f && (f = controller.textArea.value, g = !0);
    ...
    function makeInsertIconPopup(controller) {
        ...
        pop.SetWidth(400);
        pop.SetContent('<div class="bookshelf-edit-popup"><div class="bookshelf-icons">' + icons.map(function(icon) {
            return '<label class="bbcodeIcons" title="' + normalise(icon) + '">\
                      <div>\
                         <span class="bookshelf-icon-element fa fa-' + icon + '" data-icon-type="font-awesome"></span>\
                      </div>\
                      <input type="radio" value="' + icon + '" style="visibility:hidden;display:none;" name="icon"></input>\
                   </label>'
        }).join('') + '</div></div>');
        jSlim.on(pop.content, 'input', 'click', function() {
    ...
    function makeReplacePopup(target) {
        ...
        var finder = pop.content.getElementById('find');
        var replacer = pop.content.getElementById('replace');
        var findButt = pop.content.getElementById('find_button');
        var replaceButt = pop.content.getElementById('replace_button');
        jSlim.on(pop.content, '#find', 'change', function() {
            pop.content.getElementById
            findButt.dataset.nextstart = 0;
        });
        jSlim.on(pop.content, '#find_button', 'click', function() {
            var find = finder.value;
            var nextStart = parseInt(this.dataset.nextstart);
            var text = controller.textarea.value.substring(nextStart, controller.textarea.value.length);
            if (find != '') {
                ...
                this.dataset.nextstart = end;
                controller.textarea.selectionStart = start;
                controller.textarea.selectionEnd = end;
                controller.textarea.focus();
            }
        });
        jSlim.on(pop.content, '#replace_button', 'click', function() {
            var find = finder.value;
            if (find != '') {
                var start = controller.textarea.selectionStart;
                var end = controller.textarea.selectionEnd;
                if (start != end) {
                    if (start > end) {
                        var t = start;
                        start = end;
                    ...
                    var sel = controller.getText().substring(start, end);
                    var replace = replacer.value;
        ...
        jSlim.on(pop.content, '#replace_2_button', 'click', function() {
            if (finder.value != '') {
                controller.setText(replaceAll(finder.value, replacer.value, controller.getText()));
            }
        });
        pop.Show();
    }
...
function addColorTiles(controller, panel, colors) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (code != '') {
            ...
            item.addEventListener('click', function() {
                var t = controller || App.GetControllerFromElement(jSlim.nearest(document.querySelector('.active_text_area'), '.bbcode-editor'));
...
function listNames() {
    ...
    var stories;
    if (document.querySelector('.story-card-list')) {
        stories = document.querySelectorAll('.story-card-list .story-card .story_link');
    } else if (document.querySelector('.story-list')) {
        stories = document.querySelectorAll('.story-list .right .story-title');
    } else {
        stories = document.querySelectorAll('.story_container .story_name');
    }
    var len = stories.length;
    for (var i = 0; i < len; i++) {
        var name = stories[i].innerText;
        var link = stories[i].href;
        if  (name != null && name != "") {
    ...
    pop.SetContent('\
<textarea class="listText" style="min-height:300px;"></textarea>\
<div class="drop-down-pop-up-footer">\
    <label class="toggleable-switch">\
        <input class="bbc" type="checkbox"></input><a></a>\
        <span class="toggle-on">BBcode</span>\
        <span class="toggle-off">Plain Text</span>\
    </label>\
    <label class="toggleable-switch">\
        <input checked="" class="sfw" type="checkbox"></input><a></a>\
    </label>\
    <span class="toggle-on">Include Mature Stories</span>\
</div>');
    pop.SetWidth(600);
    
    
    var area = pop.content.querySelector('textarea');
    area.value = sfw;
    var toggle = pop.content.querySelector('input.bbc');
    var toggleSFW = pop.content.querySelector('input.sfw');
    
    jSlim.on(pop.content, 'input', 'change', upd);
    pop.Show();
    
    function upd() {
        if (toggle.checked) {
            area.value = toggleSFW.checked ? sfwCode : bbCode;
        } else {
            area.value = toggleSFW.checked ? sfw : result;
        }
    }
    
    function censorStory(element) {
        if (element.parentNode.querySelector('.content_rating_mature')) return true;
        var storyEntry = element.parentNode.parentNode;
        if (storyEntry.classList.contains('title')) storyEntry = storyEntry.parentNode;
        if (storyEntry.classList.contains('story_card')) {
            storyEntry = storyEntry.querySelectorAll('.info span');
        } else {
            storyEntry = storyEntry.querySelectorAll('.description .story_category');
        }
        var len = storyEntry.length;
        for (var i = 0; i < len; i++) {
            var ht = storyEntry[i].getAttribute('class');
...
function addBannerCreditsFromScratch(sets) {
    ...
    if (!$('.banner_credits').length) {
        $('.front_page').after('\
          <div class="content mobile-no-margin">\
...
function changeBanner(source, img, color, pos) {
    ...
    if (color && color.length) {
        $('.user_toolbar').attr('data-background-color', color).css('background', 'linear-gradient(to bottom, ' + toZeroAlpha(color) + ' 0%, ' + color + ' 85%)');
    }
}
...
.bbcode hr::after, .article hr::after {\
    display: none !important;}\
\
/*Now with 20% less cancer*/\
.no-lightbox [data-lightbox] {\
...
.front_page .featured_box .left ul li.selected {\
...
.all-chapters-hidden {\
    display: none;\
    text-align: center;}\
.chapters_compact .all-chapters-hidden {\
    display: block;}\
.all-chapters-hidden {\
    position: relative;\
    padding: 10px;\
...
.titleHidden header.header .theme_selector a, .titleHidden header.header .theme_selector a {\
...
.drop-size-pick ul li {\
  ...
  text-align: center;\
  border-radius: none;}\
.drop-size-pick ul li:nth-child(4) ~ li {\
  width: 19.8%;}\
...
.drop-down li.button-group > .drop-down {\
  margin-top: 100%;\
  margin-left: -320%;}\
.bbcode-editor .button-group > .drop-down ul:not(.scrollable, .drop-down-emoticons) {\
...
.compact_chapters {\
    display: none;}\
.all-shown .chapters li.hidden {\
  display: block;}\
.all-shown .chapter-expander-toggle {\
  display: none;}\
...
.user-card .top-info .button:first-child {\
...
@media all and (min-width: 700px) {\
    ...
    .user_toolbar > ul > li::after {\
        right: -1px;\
        background: rgba(150,150,150,0);//rgba(71,125,210,0);\
...
.user-page-header .tabs li.selected a {\
  z-index: 0 !important;}', 'Fimfiction_Advanced_Banner_Stylesheet');
}
...
function setPinUserbar(v) {settingsMan.setB('pin_userbar', v, false);}
...
function setWideNotes(v) {settingsMan.setB("wideAuthorNotes", v, true);}

function getSaveFocus() {return settingsMan.getB('ultra_snow_save_focus', true);}
function setSaveFocus(v) {settingsMan.setB('ultra_snow_save_focus', v, true);if (snower) snower.SetSaveFocus(v);}

function getShuffle() {return settingsMan.getB("shuffle_slideShow", true);}
function setShuffle(v) {settingsMan.setB("shuffle_slideShow", v, true);}

function getFancyBanners() {return settingsMan.getB('fancy_banners', false);}
function setFancyBanners(v) {
    settingsMan.setB('fancy_banners', v, false);
    if (v) {
        bannerScrollOn();
        updateBannerScroll();
    } else {
...
function setCustomFont(v) {
    applyCustomFont(v);
    settingsMan.set('custom_font', v, 'Default');
}
...
function setStoryWidth(v) {settingsMan.set("storyWidth", v, '46em');}
...
function setBannersEnabled(v) {
    settingsMan.setB("banners", v, true);
...
function Slider() {
    ...
    function loadImg() {
        ...
        setTimeout(function() {
            userToolbar.style.transitionDuration = '';
        }, 3000);
    ...
    function transitionStart() {
        ...
        img.addEventListsner('load', loadImg);
    }
...
function setupSweetie() {
    ...
    var imgLabels = 'Sweetie Belle;Scootaloo;Apple Bloom;Applejack;Twilight Sparkle'.split(';');
    var imgs = [
            ...
            if (belle.dragging) {
                ...
                if (passLevel == pass.length) {
                    ...
                    if (grabbedImage == null) {
                        ...
                        jSlim.all('.grabbedImage', function() {
                            me.dataset.held = false;
                            me.classList.remove('grabbedImage');
                        });
                        grabbedImage = null;
                    }
                }
...
.cursor.click:after {\
    ...
    width: 30px;\
    height: 30px;}\
.cursor.smart.click:before, .cursor.smart.click:after {\
...
@-{0}-keyframes wub {\
    ...
    function doGrab() {
        ...
        var img = document.elementFromPoint(lastClientX , lastClientY);
        belle.style.left = oldX;
        if (img && img.tagName == "IMG" && !img.classList.contains('nopickup')) {
            if (img.attr("dragged") != "true") {
                var result = img.cloneNode();
                result.setAttribute('class', 'grabbedImage');
                result.removeAttribute('id');
                result.dataset.dragged = true;
                result.dataset.held = true;
                ...
                result.addEventListener("click", function(e) {
                    if (e.shiftKey) {
                        ...
                        if (grabbedImage == this) {
                            ...
                            this.classList.add('grabbedImage');
                ...
                img.classList.add('grabbedImage');
                img.parentNode.appendChild(img);
                img.style.top = (lastY - img.clientHeight/2);
                img.style.left = (lastX - img.clientWidth/2);
                grabbedImage = img;
            }
        }
    }
    ...
    function spawnCursor(smart) {
        ...
        var cursor = $('<div class="gameObj cursor_container"><div data-level="1" class="cursor click"><img class="nopickup" src="data:image/png;base64,' + curs[0] + '" /></div><div class="label" ><div>level <span class="level">1</span></div><div><a>Sell for <span class="value">' + base + '</span></a></div></div></div>');
        if (smart) {
            cursor.find('.cursor').addClass('smart');
        }
        $('body').append(cursor);
        ...
        cursor.find('a').on('click', function(e) {
            var value = parseInt(cursor.find('.cursor').attr('data-level')) * base;
            bank += value;
    ...
    function checkClick(cursor) {
        ...
        for (var c = 0; c < cookies.length && clicks > 0; c++) {
            if (!cursor.hasClass('smart') || !cookies.eq(c).hasClass('wrath')) {
                var i = $(cookies[c]);
                var coo = i.offset();
    ...
    function buildShop(holder) {
        ...
        if (bank >= cursorCost * 2) {
            var button = $('<button class="styled_button">Buy 1 Smart Cursor (' + (cursorCost * 10) + ')</button>');
            holder.append(button);
            button.on('click', function() {
                if (bank >= cursorCost * 10) {
                    bank-= cursorCost * 10;
                    cursorCost += 16;
                    say(getPointsName() + ': ' + bank + '</br>' + getPointsName() + ' Clicked: ' + score);
                    spawnCursor(true);
...
function playerEnded(s) {\
    ...
    function help() {
        ...
        pop.SetFooter('<button class="styled_button" type="button" id="belle_type" >Switch Scepter ( <span>' + ($(belle).attr('selected_image_index') + 1) + '</span>/' + imgs.length + ' )</button><span></span>');
        pop.Show();
    ...
    function shake() {
        if ((Math.floor(Math.random()*31)) == 5) {
            ...
            $('.body_container, .footer').css('animation', 'shake_shake 0.17s linear ' + count);
            setTimeout(function() {
                $('.body_container, .footer').css('animation', '');
            },count * 170);
        }
        if (shakeCount >= 20) {
            shakeCount = -1;
    ...
    function say(text, duration, fadeOptions) {
        ...
        $('#belle .speech_container').css('display', 'block');
        ...
        if (duration != null) {
            $('#belle .speech').attr('data_timeout_function', setTimeout(function() {
                $('#belle .speech_container, #belle .options_container').css('opacity', 0);
                if (fadeOptions) {
                    $('#belle .speech').attr('data_timeout_function', '');
                }
                setTimeout(function() {
                    $('#belle .speech_container, #belle .options_container').css('display', '');
                }, 2000);
            },duration));
        }
    }
    ...
    function options(factory) {
        ...
        container.html('');
        factory(container);
        $('#belle .options_container').css({
            'opacity': container.children().length ? 1 : 0, 'display': 'block'
        });
        if (!container.children().length) {
            setTimeout(function() {
                $('#belle .options_container').css('display', '');
            }, 2000);
        }
    }
